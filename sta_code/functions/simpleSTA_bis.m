function [STA,RawMu,RawCov] = simpleSTA_bis(Stim, sp, n, CriticalSize)
...
% 2. Compute both the spike-triggered and raw stimulus means and covariances
    ...
    swid = size(Stim,2)*size(Stim,3);  %% GIULIO!!!
...
message=['chunk number 1 analyzed\n'];
fprintf(message)
...
%             STC = STC + SS(iisp,:)'*(SS(iisp,:).*repmat(spvec(iisp),1,rowlen));
            ...
            message=['chunk number ' num2str(j) ' analyzed\n'];
            fprintf(message)
        end
